...
// Normalization factors for each output channel (depends on the number of active input channels)
...
void mixer_init() {
	...
	switch(tvSettings.platform) {
		...
		if (tvSettings.bPilotCable) {
			ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][MIXER_RX_IN_FTN] = true; //in aircraft it is only true when having a three-part conference
		}
		if (tvSettings.bPilotHeadSet) {
			ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][MIXER_RX_IN_FTN] = true; // headset ls
		}
		...
		// only CLT in aircraft TODO: correct?
		if (tvSettings.bPilotCable) {
			ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][MIXER_RX_IN_CLT] = true;
		}
		if (tvSettings.bPilotHeadSet) {
			ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][MIXER_RX_IN_CLT] = false;
		}
		...
		if (tvSettings.bPilotCable) {
			ppbMixerActive[MIXER_TX_LINE_OUT_FTN][MIXER_RX_IN_PILOT] = true; //in aircraft it is only true when having a three-part conference
		}
		if (tvSettings.bPilotHeadSet) {
			ppbMixerActive[MIXER_TX_LINE_OUT_FTN][MIXER_RX_IN_PILOT] = true; // headset
		}
		...
		ppbMixerActive[MIXER_TX_RADIO_OUT_PILOT][MIXER_RX_IN_PILOT] = true;
...
//		ppbMixerActive[MIXER_LS_OUT_TECH_RIGHT_BACK][tvSettings.enMixer1stTechChannel] = false; // tech on CA is first tech.
		...
		//ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][MIXER_RX_IN_MMI_FX] = true;	// MMI not in mixer, separate volume setting
		//ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][MIXER_RX_IN_PILOT] = true;	// sidetone not in mixer, separate volume setting
		//ppbMixerActive[MIXER_LS_OUT_PILOT_MONO][tvSettings.enMixer1stTechChannel] = true; // set first tech or mich tech to pilot mono?
 ...
 * 							      aircraft cable to BS or CA
 * 				PilotToRadio	- Output
 *	\return		None
 *****************************************************************************/
void pilot_channel_selection(q15 pq15MicPilotHeadSet[], q15 pq15MicPilotCable[], q15 pq15PilotToRadio[]){
	int k;
...
void mixer_channel_setup(tvRxChannels_t rx, tvMicChannels_t mic, tvTxChannels_t tx, tvSpeakerChannels_t ls) {
	...
	ppq15mixerOut[MIXER_TX_RADIO_OUT_PILOT]			= tx.psPilotToRadio;
...
void mixer() {
	...
	for(iOut=0; iOut<NUM_CH_MIXER_OUT; iOut++) {
		if(0 != pq15MixerNormFactor[iOut]) {
			#ifdef USE_DUMPDATA
			//	mixer_out_ave[iOut]=abs(ppq15mixerOut[iOut][0]);
			#endif
