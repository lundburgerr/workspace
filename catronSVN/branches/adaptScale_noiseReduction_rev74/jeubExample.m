...
elseif true %TODO:Test this signal aswell
    ...
    y = scalefactor*y;
     yphi = pwelch(y(1:1320), [], [], 512);
%     YPSDinv = 20*log10(yphi)-mean(20*log10(yphi));
     xphi = pwelch(x(1:1320), [], [], 512);
%     XPSDinv = 20*log10(xphi)-mean(20*log10(xphi));
%     
%     yphi = yphi*10^(-mean(log10(yphi))).*10.^(-YPSDinv/20);
%     xphi = xphi*10^(-mean(log10(xphi))).*10.^(-XPSDinv/20);
end

%[s_hat, s_hatpf, db_drop, diffdiff] = jeubNoise(x, y, XPSDinv, YPSDinv, fs);
[s_hat, s_hatpf, db_drop, diffdiff] = jeubNoise(x, y, fs);
