...
[e_outWest, e_tdl] = SubSynt(YWest, N, SIMU_LENGTH-N, R, p);
[g_outWest, g_tdl] = SubSynt(GWest, N, SIMU_LENGTH-N, R, p);
...
e_outWest = fill_up_with_const(e_outWest, SIMU_LENGTH, 0);
g_outWest = fill_up_with_const(g_outWest, SIMU_LENGTH, 0);
...
if(DISTORTION_CALC)
    ...
    [sfWest, e_tdl] = SubSynt(SFWest, N, SIMU_LENGTH-N, R, p);
    sf5 = fill_up_with_const(sf5, SIMU_LENGTH, 0);
    sfWest = fill_up_with_const(sfWest, SIMU_LENGTH, 0);
    ...
    [nfWest, e_tdl] = SubSynt(NFWest, N, SIMU_LENGTH-N, R, p);
    nf5 = fill_up_with_const(nf5, SIMU_LENGTH, 0);
    nfWest = fill_up_with_const(nfWest, SIMU_LENGTH, 0);
    ...
    [sdiWest, srWest, nrWest, iSNRWest, oSNRWest] = speech_quality(xs, sfWest, sAct, xn, nfWest, 0.3);
...
dampWest = 1; %10^(-GLsub_dB/20);
damp5 = 1; % Already fixed this...

% Create audio file that compares different methods
cd(DIR_HOME)
sig = [xs, mic1, e_outWest'*dampWest, e_out5'*damp5];
gainText = ['_sGain',num2str(sgain),'_nGain',num2str(ngain),'_SNR',num2str(SNRact),'_beta',num2str(beta_rise_db_per_sec)];
dirText = [nSource{nMode},gainText];

% --  s0 = [DIR_DATA,'p501_2female_2male_ver2.wav'];
% All 4 sentences
tStart = 40+(3*4);
tStop = 55.5+(4*4);
% Four hours of ...
...
% s0 = [DIR_DATA,'s0_16kHz_nlively_disc_REVERB_WARM2.wav'];
% tStart = 20*2+3+8; % 8 s into speech sequence
% tStop = 20*3+3-3; % -3 removes silence
tSilence = 0.5;

% Create composed wav-file
result_wav(sig, fs, tStart, tStop, tSilence, [DIR_RES,'s_sn_mWest_mOur_',dirText,'.wav']);

% Create directory with separate files
mkdir([DIR_RES,dirText])
result_wav(xs, fs, tStart, tStop, 0, [DIR_RES,dirText,'\s.wav']);
result_wav(mic1, fs, tStart, tStop, 0, [DIR_RES,dirText,'\sn.wav']);
result_wav(e_outWest'*dampWest, fs, tStart, tStop, 0, [DIR_RES,dirText,'\west.wav']);
result_wav(e_out5'*damp5, fs, tStart, tStop, 0, [DIR_RES,dirText,'\prop.wav']);


% --- Paper figures ---
disp(['-- Time: ',num2str(toc),' s. ','Creating result figures'])
result_figure2(X_vec, G5tot, GWest, ceil(fs/R), DIR_RES, TMdB, 'resultMOS');
% result_figure2(X_vec, G5tot, G7tot, ceil(fs/R), DIR_RES, TMdB, 'resultMOS');


% ---< Append to text file >---
if(APPEND_TO_TXT_FILE)
    disp(['-- Time: ',num2str(toc),' s. ','Creating text file'])
    infoTxt = [s0,'_',dirText,'_',num2str(totLengthTime),'s'];
    resMat = {infoTxt,'SBA ',10*log10(iSNRWest), 10*log10(oSNRWest), 10*log10(sdiWest), 10*log10(srWest), 10*log10(nrWest)};
    resMat(2,1:length(resMat)) = {infoTxt,'Prop',10*log10(iSNR5), 10*log10(oSNR5), 10*log10(sdi5), 10*log10(sr5), 10*log10(nr5)};

    % Check if file exists
    fid = fopen([DIR_RES, 'speech_quality_beta.txt'],'rt');
    if(fid == -1)
        fid = fopen([DIR_RES, 'speech_quality_beta.txt'],'wt');
        % Write header
        fprintf(fid, 'Wav file info\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tMethod\tiSNR\toSNR\tSDI\t\tSR\t\tNR\n');
    else
        % File already exists, append results
        fclose(fid); % Close previous file pointer
        fid = fopen([DIR_RES, 'speech_quality_beta.txt'],'at');
    end

    for(i=1:2)
        % Write wav file info
        fprintf(fid, '%s\t%s', resMat{i,1}, resMat{i,2});
        % Write speech quality measures
        for(j=3:length(resMat))
            fprintf(fid, '\t%5.2f', resMat{i,j});
        end
        fprintf(fid, '\n');
    end
    fprintf(fid, '---------------------------------------------------------\n');
    fclose(fid);
end % if(APPEND_TO_TXT_FILE)

disp(['-- Time: ',num2str(toc),' s. ','Finished'])


% gamma = 0.999;subplot(2,1,1),plot(db(smoothing(g_outWest,gamma,gamma)));subplot(2,1,2),plot(db(smoothing(g_out5,gamma,gamma)))
% figure
% subplot(2,1,1), plot(db(mean(G7tot,1)),'r'), hold on, plot(db(mean(GWest,1))), legend('Prop','SBA'), title('Mean Gain')
% subplot(2,1,2), plot(db(mean(A5,1)),'k'), hold on, plot(db(mean(An5,1)),'r'), plot(db(mean(AnWest,1))), legend('A(n)','Prop','SBA'), title('Noise level estimation')
% figure, plot(db(A5(2,:)),'k'), hold on, plot(db(An5(2,:)),'r'), plot(db(AnWest(2,:))), legend('A(n)','Prop','SBA'), title('Subband k=1, Noise level estimation')
