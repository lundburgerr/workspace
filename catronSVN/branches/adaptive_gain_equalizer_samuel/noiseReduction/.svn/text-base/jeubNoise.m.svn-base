...
%     end
    ...
    %phi_pld(phi_pld<0) = 0;
    G_new = phi_pld./(phi_pld + GAMMA*phi_nn+eps);
    %G_new = filter(ones(MALENGTH,1)/MALENGTH, 1, G_new);
    %G = (1-GADAPT)*G + GADAPT*G_new;
    G = G_new;
    %G = filter(ones(9,1)/9, 1, G);
    %G(G<GAINMIN) = GAINMIN;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    Gpf = G;
    %Gpf(Gpf<GAINMIN) = GAINMIN;
    for n=1:NPSD
        if Gain(n) > Gpf(n)
            Gain(n) = Gain(n)*FALL;
            Gain(n) = max(Gpf(n), Gain(n));
        else
            Gain(n) = Gain(n)*RISE;
            Gain(n) = min(Gpf(n), Gain(n));
        end
    end
...
%     end
    %figure(13); plot(t, x1, 'r'); hold on; plot(t, x2, 'b');
