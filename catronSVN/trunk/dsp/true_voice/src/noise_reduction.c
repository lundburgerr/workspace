...
#include "dsplib.h"			// Install dsplib_c64Px_3_1_1_1_Win32.exe in default folder
#include <math.h>
#include "expexpint.h"
...
#define VAD_WAIT_TIME ((short)250) //ms
#define VAD_TIMER_NR ((short)VAD_WAIT_TIME>>2)

#define LOG2_NFFT 6 //log2(NFFT)
#define PAD 16//TODO: how large should this be?

#define PHI_MIN ((q15)6553) //6553=0.2				//Threshold for speech detection
#define PHI_MAX ((q15)26214) //26214=0.8			//Threshold for speech detection
//#define GAMMA 1					//overestimation factor for noise PSD for bad SNR
#define GAMMA 2					//overestimation factor for noise PSD for good SNR
#define ALPHA1 ((q15)22937) //22937=0.7			//Adaptation coefficient for mick PSD
#define ALPHA2 ((q15)22937) //22937=0.7			//Adaptation coefficient for noise PSD
...
#define LAMBDA_SPEECH ((q15)328) //328=0.01 			//Adaptation speed for inverse PSD
//#define GAINMIN ((q15)8192) //8192=0.25 //10^(-12/20)		//Minimum Gain for bad SNR
#define GAINMIN ((q15)2068) //2068=10^(-24/20)		//Minimum Gain for good SNR
...
#define P_FALL 27852 // 27852=0.85				//Release time
#define P_RISE  40959 //	40959=1.25		//Attack time
...
nr_status_t nr_status[NOIRED_NUM_MIC_CH];
...
static void fftReal_q16xcq16(const q15 *pq15Input, complex_q15 *pcq15Output);
static void gen_fft_realfactors(void);
static int gen_twiddle_fft16x16(q15 *w, int n);
static int gen_twiddle_ifft16x16_sa(short *w, int n);
static void ifft_cq16xcq16(const complex_q15 *pcq15Input, q15 *pq15Output);
static void linearUpdate_q31(q31 *pq31Update, const q31 *pq31With, const q15 alpha, const unsigned int uiArrayLength);
static int MA_filter(q15 output[], q15 input[], int maSize, int length);
static int MA_filter_q31_Asq15(q31 output[], q31 input[], int maSize, int length);
static void musicSupp(q15 *q15G, const q31 *q31X1mag, unsigned short usThresh, unsigned int q15Scalingfactor);
static void nr_status_init(nr_status_t *nr_status);
static void updateP(q31 *q31P, q31 *q31X1, q31 *q31X2, q15 alpha);
static void veccabs_square_shift_cq15oq31(q31 *pq31Output, const complex_q15 *pcq15z, const unsigned int uiArrayLength, unsigned int uiShift);
static void vecdiv_pos_q31xq31_Asq15(q31 *pq31Output, const q31 *pq31VecA, const q31 *pq31VecB, const unsigned int uiArrayLength);
...
static void vecmultq15xq15(q15 *pq15Output, const q15 *pq15VecA, const q15 *pq15VecB, const unsigned int uiArrayLength);
static void vecmultq15xcq15(complex_q15 *pq15Output, const q15 *pq15VecA, const complex_q15 *pq15VecB, const unsigned int uiArrayLength);
static void vecmultq31xScalar(q31 *pq31Output, const q31 *pq31Vec, const int scalar, unsigned short shift, const unsigned int uiArrayLength);
static void vecmultq31xq31_Asq15(q31 *pq31Output, const q31 *pq31VecA, const q31 *pq31VecB, const unsigned int uiArrayLength);
static void vecsub_q31(q31 *pq31Output, const q31 *pq31VecA, const q31 *pq31VecB, const unsigned int uiArrayLength);
static void voxRobin(bool *pbMute, bool *pq15speech, float *pq15Voxscale, const q31 *q31Pss, const q31 *q31Pnn, vox_status_t *vox_status, short sVadSensitivity);
...
//	}
	...
	// Initialize structs for each channel
	nr_status_init(&nr_status[NOIRED_MIC_CH_TECH]);
	nr_status_init(&nr_status[NOIRED_MIC_CH_PILOT]);
}
...
bool nr_damp(q15 *pq15x1, const q15 *pq15x2, nr_status_t *nr_status, bool bDisableVAD, short sVadSensitivity) {
	...
	//Scale down the signal 0.95 times original value to avoid clipping
	vecmultq15xScalar(pq15x1, pq15x1, 31130, 15, BUFLEN_8KHZ);
	vecmultq15xScalar(pq15x2, pq15x2, 31130, 15, BUFLEN_8KHZ);
 ...
 *	\return		None
 *****************************************************************************/
static void musicSupp(q15 *q15G, const q31 *q31X1mag, unsigned short usThresh, unsigned int scalingfactor){
	unsigned int n;
	q15 zeta = 0;
	q31 denominator = 0;
	q31 numerator = 0;
 ...
 *	\return		None
 *****************************************************************************/
static void vecmultq31xScalar(q31 *pq31Output, const q31 *pq31Vec, int scalar, unsigned short shift, const unsigned int uiArrayLength){
	unsigned int i;
	long result;
	for(i=0; i<uiArrayLength; i++){
 ...
 *	\return		None
 *****************************************************************************/
static void gen_fft_realfactors(){
	unsigned int i;
	double M = 32767.5;
	for (i = 0; i < NFFT/2; i++)
	{
...
// ---< Unused code >---
#if 0
static void vox(q15 *pq15prob, bool *pq15speech, q15 *pq15Voxscale);
static void vecmultq31xq15(q15 *pq15Output, const q31 *pq31VecA, const q15 *pq15VecB, const unsigned int uiArrayLength);
static void veccabs_square(q15 *pq15Output, const complex_q15 *pcq15z, const unsigned int uiArrayLength);
static void veccabs_square_shift_q15(q15 *pq15Output, const complex_q15 *pcq15z, const unsigned int uiArrayLength, unsigned int uiShift);
static void vecdiv_pos_q15xq15(q15 *pq15Output, const q15 *pq15VecA, const q15 *pq15VecB, const unsigned int uiArrayLength);
static void vecdiv_pos_q15xq15oq31(q31 *pq15Output, const q15 *pq15VecA, const q15 *pq15VecB, const unsigned int uiArrayLength);
static void vecdiv_pos_q31xq31(q31 *pq31Output, const q31 *pq31VecA, const q31 *pq31VecB, const unsigned int uiArrayLength);
static void vecadd_q15(q15 *pq15Output, const q15 *pq15VecA, const q15 *pq15VecB, const unsigned int uiArrayLength);
static void vecadd(int *output, const int *vecA, const int *vecB, const unsigned int uiArrayLength);
static void vecsub_q15(q15 *pq15Output, const q15 *pq15VecA, const q15 *pq15VecB, const unsigned int uiArrayLength);
static void vecshift_scalar(int *output, const int *vecA, const short shift, const unsigned int uiArrayLength);
static void linearUpdate_q15(q15 *pq15Update, const q15 *pq15With, const q15 alpha, const unsigned int uiArrayLength);
static q15 roundMap(q15 val, short shift, short max);
static int gen_twiddle_ifft16x16(short *w, int n);
 ...
 *	\return		None
 *****************************************************************************/
static void vecrecp1_q15(q15 *pq15Output, const q15 *pq15Input, const unsigned int uiArrayLength){
	unsigned int i;
	for(i=0; i<uiArrayLength; i++){
 ...
 *	\return		None
 *****************************************************************************/
static void vecaddScalar_q15(q15 *pq15Output, const q15 *pq15VecA, const q15 q15Scalar, const unsigned int uiArrayLength){
	unsigned int i;
	for(i=0; i<uiArrayLength; i++){
 ...
 *	\return		None
 *****************************************************************************/
static void vecshift_q15(q15 *pq15Output, const q15 *pq15VecA, const q15 *pq15VecShift, const unsigned int uiArrayLength){
	unsigned int i;
	for(i=0; i<uiArrayLength; i++){
		if(pq15VecShift>=0)
