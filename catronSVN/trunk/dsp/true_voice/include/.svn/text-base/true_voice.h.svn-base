...
// Enumeration of TX channels. This is necessary for disabling 1st technician transmit on base station
typedef enum {
	TV_TECH1_TO_LMR,
	TV_TECH1_TO_FTN,
	TV_TECH1_TO_LS_TECH,
	TV_TECH1_TO_LS_PILOT,
	TV_TECH1_NUM_OUT_CH		// Number of possible selections
} tech1_tx_channel_enum_t;
...
typedef struct {// Signals used on	| PT	| BS	| CA	|
	...
	short *psTech6;				//	| Yes	| Yes	| Yes	|
	short *psTech7;				//	| Yes	| Yes	| Yes	|
	short *psTech8;				//	| Yes	| Yes	| Yes	|
...
typedef struct {// Signals used on	| PT	| BS	| CA	|
	...
	short *psPilotCable;		//	| No	| Yes	| Yes	|	Microphone signal from pilot (+CLT+sidetone) in aircraft via cable J8
	short *psPilotCltCable;		//	| No	| Yes	| No	|	Microphone signal from pilot in aircraft via cable J9
} tvMicChannels_t;
// Pointer to audio buffers for transmit channels
typedef struct {// Signals used on	| PT	| BS	| CA	|
	short *psTechToRadio;		//	| Yes	| Yes	| Yes	|	Processed technician microphone signal
	short *psPilotAndCltToRadio;//	| No	| Yes	| No	|	Processed CLT and pilot microphone (taken from either aircraft cable or pilot headset to be sent to 1st tech PT)
...
typedef struct {// Signals used on	| PT	| BS	| CA	|
	...
	short *psPilotMono;			//	| No	| Yes	| Yes	|	Pilot always hear mono audio. Headset connected to jack on BS or CA.
	short *psPilotMonoCable;	//	| No	| Yes	| Yes	|	Headset signal to pilot headset in aircraft via J8 BS-Pilot cable
	short *psPilotMonoCltCable;	//	| No	| Yes	| No 	|	Headset signal to pilot headset in aircraft via J9 CLT-Pilot cable
} tvSpeakerChannels_t;
...
/*****************************************************************************
 *	\brief		Specify the tone and pause duration and volume for DTMF signals
 ...
 *	\return		None
 *****************************************************************************/
void true_voice_dtmf_settings(short, short, short);
