...
static SOCKET xSocket;
extern uint16_t configured_console_in_port;
extern uint16_t configured_console_out_port;

static int xClientAddressLength = sizeof( struct sockaddr_in );
...
SOCKET xSocket = INVALID_SOCKET;
	...
	else
	{
		...
		else
		{
			...
			xServer.sin_port = htons( configured_console_in_port );
...
void console_send(char *text)
{
   ...
   /* memset( asciihex, 0x00, 256);*/
    ...
    xClient.sin_port = htons(configured_console_out_port);
    ...
    sendto( xSocket, (const char *) text,  strlen(text), 0, ( SOCKADDR * ) &xClient, xClientAddressLength );
